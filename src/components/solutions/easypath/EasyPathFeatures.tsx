...
export default function EasyPathFeatures() {
  ...
  return (
    <section ref={ref} className="relative py-28 lg:py-36 bg-white overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_100%,rgba(233,213,255,0.2),transparent)]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, i) => {
            ...
            return (
              ...
              >
                <div className="relative h-full p-7 rounded-2xl border border-slate-100 bg-white hover:border-slate-200 hover:shadow-xl hover:shadow-purple-100/50 transition-all duration-500">
                  {/* Hover gradient top */}
                  <div className="absolute top-0 left-0 right-0 h-[2px] rounded-t-2xl bg-gradient-to-r from-purple-500 to-violet-500 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

                  {/* Number */}
                  <span className="absolute top-5 right-6 font-mono text-[4rem] font-bold text-slate-50 leading-none select-none group-hover:text-purple-50 transition-colors duration-500">
                    {String(i + 1).padStart(2, "0")}
                  </span>

                  {/* Icon */}
                  <div className="relative w-10 h-10 rounded-lg bg-slate-50 group-hover:bg-purple-50 flex items-center justify-center mb-5 transition-colors duration-500">
                    <Icon size={20} className="text-slate-400 group-hover:text-purple-500 transition-colors duration-500" />
                  </div>
